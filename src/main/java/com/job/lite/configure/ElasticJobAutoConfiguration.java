	...
	public void init() {
		...
		for (ElasticJob elasticJob : elasticJobMap.values()) {
			...
			if (Objects.isNull(jobEventRdbConfiguration)) {
				new SpringJobScheduler(elasticJob, zookeeperRegistryCenter, liteJobConfiguration, elasticJobListeners).init();
			} else {
				new SpringJobScheduler(elasticJob, zookeeperRegistryCenter, liteJobConfiguration, jobEventRdbConfiguration, elasticJobListeners).init();
			}
		}
	}
